...
convolutionD xs ys = fst . walk $ xs
 where
  walk []       = ([], ys)
  walk (x : xs) = let (zs, y : ys) = walk xs in ((x, y) : zs, ys)
...
cnv3 (xs, ys) = snd . walk $ xs
 where
  walk []       = (ys, [])
  walk (x : xs) = let (y : ys, r) = walk xs in (ys, (x, y) : r)
...
tabaRev xs = snd . walk $ xs
 where
  walk []       = (xs, [])
  walk (_ : xs) = let (y : ys, r) = walk xs in (ys, y : r)
...
catalan m = iter (1, [1])
 where
  cat a = snd . walk $ a
   where
    walk []       = (a, 0)
    walk (n : ns) = let (n' : ns', r) = walk ns in (ns', r + (n * n'))
...
cnvHalves (xs, n) = snd . walk $ (n, xs)
 where
  walk (0, xs    ) = (xs, [])
  walk (n, x : xs) = let (y : ys, r) = walk (n - 2, xs) in (ys, (x, y) : r)
